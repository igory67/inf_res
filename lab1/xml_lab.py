...
for obj in objects:
    for atr in obj:
        atr_name = atr.tag
        xml_type = atr.get('type')
        
        if xml_type is None:
            xml_type = 'string'
        text = atr.text

        if atr_name not in info_dir:
            info_dir[atr_name] = {
                    'xml_type' : xml_type,
                    'is_string' : xml_type == 'string',
                    'max_len_string' : 0,
                    'empty' : True,
                    'values' : set(),
                    }

        info = info_dir[atr_name]
        if info['xml_type'] and xml_type == 'string' and text is not None:
            length = len(text)
            info['max_len_string'] = max(info['max_len_string'], length)
        
        if text is not None:
            info['empty'] = False
        
        elif len(info['values']) < 10:
            info['values'].add(text)
